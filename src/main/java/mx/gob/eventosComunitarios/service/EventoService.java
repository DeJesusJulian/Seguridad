...
boolean vuelta = false;
		...
		try {
			...
			if(event.getNombreCliente().length()>3 && event.getApellidoCliente().length()>3 && event.getTitulo().length()>3 && event.getDescripcion().length()>3 && event.getEmailCliente().length()>3 && event.getTelefono().length()>3 && event.getFechaInicio().length()>3 && event.getFechaFin().length()>3F) {
				...
				if(anio == anioF ) {
					...
					if(mes <= mesF) {
						if(dia < diaF) {
							...
							jdbcTemplate.update(sql, event.getTitulo(), event.getDescripcion(), event.getNombreCliente(), event.getApellidoCliente(), event.getEmailCliente(), event.getTelefono(), event.getFechaInicio(), event.getFechaFin(), event.getLugar());
...
boolean vuelta = false;
				...
				if(anio == anioF ) {
					...
					if(mes <= mesF) {
						if(dia < diaF) {
							...
							jdbcTemplate.update(sql, event.getTitulo(), event.getDescripcion(), event.getNombreCliente(), event.getApellidoCliente(), event.getEmailCliente(), event.getTelefono(), event.getFechaInicio(), event.getFechaFin(), event.getLugar(), event.getIdEvento());
	...
	public Evento findOne(long id) {
		...
		return this.jdbcTemplate.queryForObject(sql, new Object[] {id}, BeanPropertyRowMapper.newInstance(Evento.class));
	}
